...
RUN_TAG <- if (QUICK_TEST) "quick_test" else "full"
...
pareto_settings <- data.frame(
  ...
  ax1 = c(1, 1, 1, 0.5, 0.5),
  ax2 = c(1, 2, 5, 1, 10),
  tx = c(0.5, 0.5, 0.2, -0.5, -0.9),
  ly1 = c(1, 2, 5, 1, 1),
  ly2 = c(1, 1, 1, 1, 1),
  ay1 = c(1, 2, 2, 0.5, 15),
  ay2 = c(1, 1, 2, 0.5, 0.2),
  ty = c(0.5, 0.5, 0.5, -0.5, -0.1),
  lz1 = c(1, 2, 1, 1, 0.2),
  lz2 = c(1, 2, 5, 1, 1),
  az1 = c(1, 1, 5, 5, 5),
  az2 = c(1, 1, 1, 1, 0.1),
  tz = c(-0.5, -0.5, 0.9, -0.2, 0.9),
  stringsAsFactors = FALSE
)
  ...
  for (setting_index in seq_len(nrow(pareto_settings))) {
    ...
    for (design_index in seq_len(nrow(sample_sizes))) {
      ...
      checkpoint_name <- sprintf(
        "FGMPM_setting_%02d_design_%02d.rds",
        setting_id,
        design_id
      )
...
simulation_output <- run_pareto_all()
pareto_result <- simulation_output$summary
...
pareto_table <- make_paper_table(pareto_result)
...
openxlsx::addWorksheet(workbook, "Table 3 Pareto")
...
openxlsx::writeData(workbook, "Table 3 Pareto", pareto_table)
